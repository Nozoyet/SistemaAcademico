      ...
      {steps.map((s,i)=>(
        <div key={s.n} style={{display:"flex",alignItems:"center",flex:i<2?1:"none"}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
            <div style={{width:36,height:36,borderRadius:"50%",fontWeight:700,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"background .3s",
...
function Btn({children,onClick,variant="primary",disabled,type="button",style}){
  ...
  return(
    <button type={type} onClick={onClick} disabled={disabled} style={{...vs[variant],
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
        ...
        {electivasDisponibles.length>0&&(
          <Btn variant="ghost" onClick={()=>setShowExtraForm(p=>!p)}>
            {showExtraForm?"✕ Cerrar":"+ Añadir Electiva"}
          </Btn>
        )}
      </div>
      <Alert msg={error}/>
        ...
        <div style={{marginBottom:16}}>
          ...
          {extras.map(e=>(
            <div key={e.id} style={{display:"flex",alignItems:"center",gap:12,background:C.amberDim+"66",border:`1px solid ${C.amber}33`,borderRadius:8,padding:"10px 14px",marginBottom:6}}>
              ...
              <Btn variant="danger" onClick={()=>setExtras(p=>p.filter(x=>x.id!==e.id))} style={{padding:"4px 10px",fontSize:12}}>✕ Quitar</Btn>
            </div>
          ))}
        </div>
      )}
  ...
  return(
    <div>
      ...
      {showFinishModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.45)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20}}>
          <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:36,width:"100%",maxWidth:480,boxShadow:C.shadow,textAlign:"center"}}>
            <div style={{width:68,height:68,margin:"0 auto 18px",borderRadius:"50%",background:C.greenDim,display:"flex",alignItems:"center",justifyContent:"center",fontSize:30}}>✓</div>
                ...
                <div style={{borderTop:`1px solid ${C.border}`,background:C.surface}}>
                  {cursos.map(c=>(
                    <div key={c.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 16px",borderBottom:`1px solid ${C.border}22`,flexWrap:"wrap"}}>
                      ...
                      <Btn variant="ghost" onClick={()=>{setModal(m);setEditando(c);}} style={{padding:"4px 10px",fontSize:11}}>✏️ Editar</Btn>
                      <Btn variant="danger" onClick={()=>setDeleteConfirm({id:c.id,idMateria:m.id,grupo:c.codigoGrupo})} style={{padding:"4px 10px",fontSize:11}}>🗑</Btn>
                    </div>
                  ))}
                </div>
              )}
      ...
      <div style={{display:"flex",justifyContent:"flex-start",marginTop:24}}>
        <Btn variant="ghost" onClick={()=>setConfirmBack(true)}>← Atrás</Btn>
      </div>
    </div>
  );
}
...
export default function GestionCursos(){
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans','Segoe UI',sans-serif"}}>
      <header style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"14px 32px",display:"flex",alignItems:"center",gap:16,position:"sticky",top:0,zIndex:10}}>
        <button onClick={()=>navigate("/admin/bienvenida")} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",gap:6}}>← Volver</button>
