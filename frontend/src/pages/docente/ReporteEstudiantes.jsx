...
export default function ReporteEstudiantes() {
  ...
  const generarPreview = async () => {
    try {
      ...
      const res = await api.get("/docente/reportes/estudiantes/preview", { params: filtros });
  ...
  const confirmarDescarga = async () => {
    ...
    try {
      ...
      const res = await api.get(`/docente/reportes/estudiantes/${modalType}?${params}`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `reporte-estudiantes.${ext}`);
  ...
  const columns = [
    ...
    { key: "estudiante", label: "Estudiante", width: "28%" },
    { key: "curso", label: "Curso", width: "24%", render: (r) => r.curso?.materia?.nombre || r.curso || "—" },
    { key: "estado", label: "Estado", width: "12%", align: "center" },
  ];

  const infoItems = [{ label: "Total Estudiantes", value: preview.length }];
      ...
      <main style={s.main}>
        <div style={{ ...s.heroCard, borderTop: `4px solid ${C.color}` }}>
          <div style={{ ...s.chip, background: C.accent, color: C.color }}>
            <i className="bi bi-people-fill" style={{fontSize:14}}></i> Reporte de Estudiantes
          </div>
          <h1 style={s.heroTitle}>Reporte de <span style={{ color: C.color }}>Estudiantes</span></h1>
          <p style={s.heroDesc}>Consulta estudiantes inscritos en tus cursos asignados usando filtros.</p>
        ...
        {generado && preview.length === 0 && (
          <div style={s.emptyCard}>
            ...
            <p style={s.emptyDesc}>No se encontraron estudiantes con los filtros seleccionados.</p>
          </div>
        )}
      </main>
