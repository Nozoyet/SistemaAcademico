...
export default function ReporteMaterias() {
  ...
  const confirmarDescarga = async () => {
    ...
    try {
      ...
      const res = await api.get(`/docente/reportes/materias/${modalType}?${params}`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `reporte-materias.${ext}`);
  ...
  const totalCreditos = preview.reduce((s, r) => s + (Number(r.creditos) || 0), 0);

  const columns = [
    { key: "codigo", label: "Código", width: "15%" },
    { key: "nombre", label: "Nombre", width: "45%" },
    { key: "creditos", label: "Créditos", width: "15%", align: "center" },
    { key: "semestre", label: "Semestre", width: "15%", align: "center", render: (r) => `${r.semestre}°` },
  ];

  const infoItems = [
    { label: "Total Materias", value: preview.length },
    { label: "Total Créditos", value: totalCreditos },
  ];
      ...
      <main style={s.main}>
        <div style={{ ...s.heroCard, borderTop: `4px solid ${C.color}` }}>
          <div style={{ ...s.chip, background: C.accent, color: C.color }}>
            <i className="bi bi-book-fill" style={{fontSize:14}}></i> Reporte de Materias
          </div>
          <h1 style={s.heroTitle}>Reporte de <span style={{ color: C.color }}>Materias</span></h1>
          <p style={s.heroDesc}>Consulta las materias asignadas filtrando por periodo, carrera o nombre.</p>
          ...
          <div style={s.filterSection}>
            <div style={s.filterRow}>
              ...
              <div style={s.filterGroup}>
                <label style={s.filterLabel}>Carrera</label>
                <input name="carrera_id" value={filtros.carrera_id} onChange={change} placeholder="ID carrera" style={s.filterInput} />
              </div>
              <div style={s.filterGroup}>
            ...
            <div style={s.filterRow}>
              <div style={s.filterGroup}>
                ...
                <input name="nombre" value={filtros.nombre} onChange={change} placeholder="Nombre materia" style={s.filterInput} />
              </div>
              <div style={s.filterGroup}>
        ...
        {generado && preview.length === 0 && (
          <div style={s.emptyCard}>
            ...
            <p style={s.emptyDesc}>No se encontraron materias con los filtros seleccionados.</p>
          </div>
        )}
      </main>
