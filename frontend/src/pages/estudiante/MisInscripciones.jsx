...
function Tag({ children, color = C.accent }) {
  return <span style={{ background: color + "18", color, border: `1px solid ${color}33`, borderRadius: 999, padding: "2px 10px", fontSize: 11, fontWeight: 600 }}>{children}</span>;
}
...
function HorarioChip({ h }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, background: C.accentDim, borderRadius: 8, padding: "6px 12px" }}>
      <span style={{ fontWeight: 700, fontSize: 12, color: C.accent, width: 70 }}>{h.diaSemana}</span>
      <span style={{ fontSize: 13, color: C.text }}>{h.horaInicio} – {h.horaFin}</span>
      <span style={{ fontSize: 11, color: C.textSub, marginLeft: 4 }}>🏫 {h.aula}{h.edificio ? ` · ${h.edificio}` : ""}</span>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
              <span style={{ fontSize: 16, fontWeight: 700, color: C.text }}>{materia?.nombre}</span>
          ...
          <button onClick={() => setExpanded(p => !p)} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "5px 10px", cursor: "pointer", color: C.textMuted, fontSize: 12, flexShrink: 0 }}>
            {expanded ? "▲" : "▼"}
          </button>
        </div>
...
function VistaHorario({ inscripciones }) {
  ...
  // Construir mapa de bloques: dia -> [{horaInicio, horaFin, nombre, color}]
  const bloques = {};
  ...
  const minBase = toMin("07:00");
  const totalMin = toMin("22:00") - minBase;
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden", marginBottom: 24 }}>
      <div style={{ padding: "14px 18px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ fontSize: 14, fontWeight: 700, color: C.text }}>📅 Mi horario semanal</span>
      </div>
      <div style={{ overflowX: "auto" }}>
        <div style={{ minWidth: 500, padding: "16px" }}>
          <div style={{ display: "grid", gridTemplateColumns: `52px repeat(${diasConClases.length}, 1fr)`, gap: 4 }}>
            ...
            <div />
            {diasConClases.map(d => (
              <div key={d} style={{ textAlign: "center", fontSize: 11, fontWeight: 700, color: C.accent, textTransform: "uppercase", letterSpacing: "0.06em", padding: "4px 0" }}>{d.slice(0, 3)}</div>
            ))}

            {/* Filas de horas */}
            {horas.map((hora, hi) => (
              <>
                <div key={hora + "h"} style={{ fontSize: 10, color: C.textMuted, textAlign: "right", paddingRight: 8, paddingTop: 2 }}>{hora}</div>
                  ...
                  if (bloque) {
                    ...
                    return (
                      <div key={dia + hora} style={{
                        gridRow: `span ${rows}`, background: bloque.color + "22", border: `1.5px solid ${bloque.color}55`,
                        borderRadius: 8, padding: "6px 8px", fontSize: 10, color: bloque.color, fontWeight: 600, overflow: "hidden",
                      }}>
      ...
      <div style={{ padding: "16px 20px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
          ...
          <div style={{ textAlign: "center", flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 10, color: C.textMuted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>Nota</div>
          </div>
        ...
        <div style={{ display: "flex", gap: 16, marginTop: 12, flexWrap: "wrap" }}>
          ...
          {curso?.horarios?.map((h, i) => (
            <span key={i} style={{ fontSize: 11, color: C.gray, background: C.grayDim, borderRadius: 6, padding: "2px 8px", fontWeight: 500 }}>
              {h.diaSemana.slice(0, 3)} {h.horaInicio}–{h.horaFin}
            </span>
          ))}
        </div>
...
function SeccionHistorial() {
  ...
  if (loading) return <div style={{ textAlign: "center", padding: 40, color: C.textMuted }}>Cargando historial…</div>;
  ...
  return (
    <div>
      ...
      <div style={{ background: "white", border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 16px", marginBottom: 16, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {/* Por semestre */}
        {semestres.length > 0 && (
          <select value={semFiltro} onChange={e => setSemFiltro(e.target.value)} style={{ padding: "6px 12px", border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 12, color: C.textSub, outline: "none", background: "white", marginLeft: "auto" }}>
      ...
      {filtrado.length === 0 ? (
        <div style={{ textAlign: "center", padding: 40, color: C.textMuted }}>
          ...
          <p style={{ margin: 0 }}>No hay registros con los filtros seleccionados</p>
        </div>
      ) : (
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
      <header style={{ background: "white", borderBottom: `1px solid ${C.border}`, padding: "14px 28px", display: "flex", alignItems: "center", gap: 16, position: "sticky", top: 0, zIndex: 10, boxShadow: "0 1px 8px rgba(0,0,0,0.06)" }}>
        ...
        {tab === "activas" && (
          <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
            {[["lista", "☰ Lista"], ["horario", "📅 Horario"]].map(([v, l]) => (
              <button key={v} onClick={() => setVista(v)} style={{
                padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer",
                background: vista === v ? C.accent : "transparent", color: vista === v ? "white" : C.textMuted,
                border: `1.5px solid ${vista === v ? C.accent : C.border}`,
              }}>{l}</button>
            ))}
          </div>
        )}
      ...
      <main style={{ maxWidth: 860, margin: "0 auto", padding: "28px 20px" }}>
        ...
        {/* Tabs principales */}
        <div style={{ display: "flex", gap: 0, marginBottom: 24, background: "white", border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden" }}>
          {[["activas", "📋 Inscripciones actuales"], ["historial", "🎓 Historial académico"]].map(([v, l]) => (
            <button key={v} onClick={() => setTab(v)} style={{
              ...
              color: tab === v ? "white" : C.textMuted,
              borderRight: v === "activas" ? `1px solid ${C.border}` : "none",
          ...
          <>
            ...
            {loading ? (
              <div style={{ textAlign: "center", padding: 60, color: C.textMuted }}>
                ...
                <p style={{ margin: 0 }}>Cargando tus inscripciones…</p>
              </div>
            ) : error ? (
              ...
              <div style={{ textAlign: "center", padding: 60 }}>
                ...
                <p style={{ fontSize: 16, fontWeight: 600, color: C.text, margin: "0 0 8px" }}>No tienes inscripciones aún</p>
                <p style={{ fontSize: 13, color: C.textMuted, margin: "0 0 20px" }}>Explora los cursos disponibles e inscríbete.</p>
                <button onClick={() => navigate("/estudiante/cursos")} style={{ padding: "10px 24px", borderRadius: 10, border: "none", background: C.accent, color: "white", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
